...
class Files(BaseHandler):
    def get(self):
        ...
        self.write(escape.json_encode(self.application.file_lister.files))
...
class WebsocketWPipe(sockjs.tornado.SockJSConnection):
    def __init__(self, *args, **kw):
        super(WebsocketWPipe, self).__init__(*args, **kw)
...
class WPipeApplication(BaseApplication):
    def setup_routes(self):
        routes = [
            [r'/assets/(.*)', NonCachingStaticFileHandler, {'path': os.path.join(self.here, 'assets/')}],
            [r'/', Index, {'template': 'wpipe.html'}],
        ]

        WebsocketWPipe.application = self
        ws_handler = sockjs.tornado.SockJSRouter(WebsocketWPipe, os.path.join('/', self.relative_root, 'ws'))
        routes += ws_handler.urls
        return routes, ws_handler
