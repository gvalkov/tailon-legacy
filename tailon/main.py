...
#-----------------------------------------------------------------------------
# Option parsing: wpipe
#-----------------------------------------------------------------------------
def parseopts_wpipe(args=None):
    description = '''
    ...
    epilog = '''
    Example command-line:
      tail -f /var/log/messages | wpipe -b localhost:8080 | nl
      wpipe /var/log/messages
    '''
...
# Main: wpipe
#-----------------------------------------------------------------------------
def main_wpipe(argv=sys.argv):
    parser, opts = parseopts_wpipe()

    if opts.debug:
        enable_debugging()

    template_dir, assets_dir = get_resource_dirs()
    config = setup_wpipe(opts)
    ...
    application = server.WPipeApplication(config, client_config, template_dir, assets_dir)
    start_server(application, config, client_config)
